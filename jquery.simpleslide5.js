//
//
//	simpleSlide5, a HTML5 valid fork of simpleSlide v1.6
//
//  G. Kocher  //  github [.] com [/] gkchr
//
//
//	simpleSlide5 is a jQuery plugin that addresses the problem of lack of designer control over
...
//	----------
//
//	simpleSlide5 is instantiated, at minimum, like this:
//	
//	simpleSlide5();
//
//	Upon activating this function, simpleSlide5 goes into your DOM and finds several key
//	elements. The simpleSlide5 HTML structure (and key elements) will look like this:
//
//	<div class="simpleSlide5-window" data-group="group_name">
//		<div class="simpleSlide5-tray" data-group="group_name">
//			<div class="simpleSlide5-slide" data-group="group_name">
//				/* Individual slide content. If content is merely an image, simpleSlide5 will
...
//				 * height. You do not, however, need to define the "simpleSlide5-slide" class'
...
//	</div>
//
//	The user may also instantiate a graphical representative element vis-a-vis the simpleSlide5
...
//
//	<div class="simpleSlide5Status-tray" data-group="group_name">
//		<div class="simpleSlide5Status-window" data-group="group_name"></div>
//	</div>
//	
//	Please visit http://www.simplesli.de and click on "features" for more configuration information.
//
//	It should be noted, however, that after simpleSlide5() has processed this structure, each slide
...
//	the window (and accompanying elements) you wish for them to control. simpleSlide5() automatically
...
//	
//	<div class="left-button" data-group="group_name"></div>
//	<div class="right-button" data-group="group_name"></div>
//  <div class="jump-to" data-group="group_name" data-nr="1"></div>
//
...
//										   work with more than one simpleSlide5 window per page.
//		'swipe' : 'false', 				// String. 'true' turns on swipe functionality for touch devices.
//		'callback': 'function()'		// String. Sets callback to actuate after simpleSlide5 initial config.
//	});
//
//	If you have any further questions on the usage of simpleSlide5, or suggestions on making it better,
...
function simpleSlide5(incoming_options) {
	jQuery(function($) {
		var options = {
			'status_width': 20,
		...
		$('.simpleSlide5-slide').css('opacity', '0');
		
		$('.simpleSlide5-tray').css('margin', '0');
		
		$('.simpleSlide5-window').prepend('<span id="ssLoading" style="color: #808080;font-family:Helvetica, Arial, sans-serif;font-size: 12px; margin: 10px 0 0 10px;display: block">Loading slides...</span>');
		
		var no_of_images = $('.simpleSlide5-slide img').size();
			
		if(no_of_images > 0) {
			
			var images = new Array();
			var i = 0;
			$('.simpleSlide5-slide img').each( function() {
...
function ssInit(){
	/* Set the dimensions of each simpleSlide5 window and tray
	...
	jQuery(function($) {
		$('.simpleSlide5-window').each( function() {	
			...
			var slide_count = $(this).find('.simpleSlide5-slide').size();		
			$(this).find('.simpleSlide5-slide').css('display','block');
			var window_height = $(this).find('.simpleSlide5-slide').first().outerHeight();

			if($.ss_options.fullscreen == 'true'){
				fullScreen(this);
			};			

			$(this).find('.simpleSlide5-slide').css({
				'display':'inline',
				'float':'left'
			});

			var window_width = $(this).find('.simpleSlide5-slide').first().outerWidth();		
			
			var window_rel = $(this).attr('data-group');
			...
			setsimpleSlide5Status(window_rel, window_height, window_width, slide_count);
			...
			$(this).find('.simpleSlide5-slide').animate({
				'opacity': '1'
			}, 300, "swing");
		
		});
		
		/* Fire callback after completion of image load and simpleSlide5 initialization */
		if(typeof($.ss_options.callback) == 'function'){
			$.ss_options.callback.call(this);
		};
		
		/* Gives each slide an 'data-nr' with the slide number */
		function setPaging(this_window) {
			var page_count = 1;
			
			$(this_window).find('.simpleSlide5-slide').each( function() {
				$(this).attr('data-nr', page_count);
				page_count++;
			});
		};
		...
		function fullScreen(this_s_window){
			...
			if(window_prop > image_prop){
				...
				$(this_s_window).find('.simpleSlide5-slide').css({
					'width': ww,
					'height': new_height,
					'overflow': 'hidden'
				});				
			...
			$(this_s_window).find('.simpleSlide5-tray').css('marginLeft', 0);
		};
		...
		function setTraySize(slideWindow, count, viewer_width) {
			...
			$(slideWindow).find('.simpleSlide5-tray').css({
				'width': slider_width + 'px'
			});
			
			$(slideWindow).find('.simpleSlide5-slide').css('display','inline-block');
		 ...
		 */
		function setsimpleSlide5Status(this_rel, height, width, image_count) {
				
			var ratio = $.ss_options.status_width/width;
			var status_height = ratio*height;
			
			$('.simpleSlide5Status-tray[data-group="' + this_rel + '"]')
				.css({
			...
			$('.simpleSlide5Status-window[data-group="' + this_rel + '"]')
				.css({
			...
			if(jQuery.support.opacity){
				$('.simpleSlide5Status-window .simpleSlide5Status-tray[data-group="' + this_rel + '"]')
				   .css({
					   'opacity': '.5',
					   'background-color': $.ss_options.status_color_inside
				});
			...
			if(!jQuery.support.opacity){
				$('.simpleSlide5Status-window .simpleSlide5Status-tray[data-group="' + this_rel + '"]').css({
		...
		$('.left-button, .right-button, .jump-to').live('click', function() {
			...
			var rel = $(this).attr('data-group');
			
			if (!$('div.simpleSlide5-tray[data-group="' + rel + '"]').is(':animated')) {
				simpleSlide5Action(this, rel);
			};
		});
	});
};
...
function simpleSwipe(this_window) {
	...
	var defaults = {
		...
		swipeLeft: function() {
			simpleSlide5Action('.right-button', $(this_window).attr('data-group'));
		},
		swipeRight: function() {
			simpleSlide5Action('.left-button', $(this_window).attr('data-group'));
		},
...
function simpleSlide5Action(action, rel_no) {
	jQuery(function($) {	
		var move_speed = $.ss_options.set_speed;
		var image_count = $('.simpleSlide5-window[data-group="' + rel_no + '"]').find('.simpleSlide5-slide').size();
		var window_width = $('.simpleSlide5-window[data-group="' + rel_no + '"]').innerWidth();
		var status_window_width = $('.simpleSlide5Status-window[data-group="' + rel_no + '"]').innerWidth();
		var status_tray_width = status_window_width * image_count;
		var current_tray_margin = parseInt($('.simpleSlide5-tray[data-group="' + rel_no + '"]').css('marginLeft'), 10);
		var current_status_window_margin = parseInt($('.simpleSlide5Status-tray .simpleSlide5Status-window[data-group="' + rel_no + '"]').css('marginLeft'), 10);
		var current_status_tray_margin = parseInt($('.simpleSlide5Status-window .simpleSlide5Status-tray[data-group="' + rel_no + '"]').css('marginLeft'), 10);
		
		if($(action).is('.jump-to')) {
			var to_page = $(action).attr('data-nr');
		...
		function move(new_margin, new_swindow_margin, new_stray_margin) {
			$('.simpleSlide5-tray[data-group="' + rel_no + '"]').animate({
				'marginLeft': new_margin
			}, move_speed, "swing");
			
			$('.simpleSlide5Status-window .simpleSlide5Status-tray[data-group="' + rel_no + '"]')
				.animate({
					'marginLeft': new_stray_margin				 
			}, move_speed, "swing");
			
			$('.simpleSlide5Status-tray .simpleSlide5Status-window[data-group="' + rel_no + '"]').animate({
